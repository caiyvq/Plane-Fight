import { _decorator, Component, Enum, EventTouch, Input, input, instantiate, math, Node, Prefab, Vec3 } from 'cc';
...
export class player extends Component {
    ...
    onTouchMove(event: EventTouch) {
        //console.log(event);
        let p = new Vec3(this.node.position.x + event.getDeltaX(), this.node.position.y + event.getDeltaY());
        this.node.setPosition(Math.min(Math.max(p.x, this.leftBound), this.rightBound), Math.min(Math.max(p.y, this.downBound), this.upBound));
    }
    ...
    oneShoot() {
        let bullet1 = poolManager.instance().getNode(this.bullet1Prefab, this.bulletParent);
        bullet1.setWorldPosition(this.bullet1Position.worldPosition);
    }
    twoShoot() {
        let bullet2_1 = poolManager.instance().getNode(this.bullet2Prefab, this.bulletParent);
        bullet2_1.setWorldPosition(this.bullet2Position1.worldPosition);

        let bullet2_2 = poolManager.instance().getNode(this.bullet2Prefab, this.bulletParent);
